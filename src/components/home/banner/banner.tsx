...
const SVGIllustration = (props: any) => {
    return (
...
const Banner = (props: { scrollRef: any }) => {
    ...
    * */
    ...
    const handleInsetScaling = () => {
        ...
        if (bannerParentRef?.current && bannerRef?.current) {
            if (percent >= 0) {
                ...
                const value = isMobileDevice() ? insetValue : insetValue / 1.5;

                // Instead of 'inset', use 'top', 'right', 'bottom', 'left'
                bannerRef.current.style.top = `${value}px`;
                bannerRef.current.style.right = `${value}px`;
                bannerRef.current.style.bottom = `${value}px`;
                bannerRef.current.style.left = `${value}px`;
    ...
    return (
        ...
        >
            {!isMobile ?
                <div className="banner-parent hidden sm:block" ref={bannerParentRef}>
                ...
                <div className="sm:hidden p-5">
                    <section className="rounded-3xl p-5 bg-black text-center">
                        <SVGIllustration/>
                    </section>
                </div>
            }
